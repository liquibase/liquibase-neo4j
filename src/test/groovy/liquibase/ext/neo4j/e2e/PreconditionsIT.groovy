...
class PreconditionsIT extends Neo4jContainerSpec {
    ...
    def "runs migrations depending on procedure existence"() {
        ...
        def command = new CommandScope(UpdateCommandStep.COMMAND_NAME)
                ...
                .addArgumentValue(DbUrlConnectionArgumentsCommandStep.USERNAME_ARG, "neo4j")
                .addArgumentValue(DbUrlConnectionArgumentsCommandStep.PASSWORD_ARG, PASSWORD)
                .addArgumentValue(DatabaseChangelogCommandStep.CHANGELOG_FILE_ARG, "/e2e/preconditions/procedureExistsChangeLog.${format}".toString())
                .setOutput(System.out)
        command.execute()

        expect:
        def row = queryRunner.getSingleRow("""
            MATCH (n:Neo4j) RETURN n.createdWhenDbLabelsExists AS createdWhenDbLabelsExists, n.createdWhenFakeProcedureExists AS createdWhenFakeProcedureExists
        """)

        row["createdWhenDbLabelsExists"] == true
        row["createdWhenFakeProcedureExists"] == null
